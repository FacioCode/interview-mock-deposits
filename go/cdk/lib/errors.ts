...
export const GO_DEFAULT_ERROR_PATTERN = '{ $.level = "error" }'
export const GO_PANIC_ERROR_PATTERN = '{ $.level = "panic" }'
...
export const CriticalAlarmConfig = {
  errorPattern: GO_DEFAULT_ERROR_PATTERN,
  threshold: 95,
  isCritical: true,
  evaluationPeriods: 4,
...
const metricNamespace = 'InterviewMockDepositsGo'
...
function definePanicAlarm (scope: Construct, name: string, lambdaFunc: awsLambda.Function) {
  const panicFilter = new awsLogs.MetricFilter(scope, name + 'PanicMetricFilter', {
    ...
    filterPattern: awsLogs.FilterPattern.literal(GO_PANIC_ERROR_PATTERN),
    metricNamespace
  })
